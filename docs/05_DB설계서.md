...
> **버전**: v1.1 | **작성일**: 2026-07-03 | **기반 문서**: PRD v1.0, 시스템 아키텍처 설계서 v1.0
...
erDiagram
  ...
  SESSIONS ||--o{ POSTS : creates
  POSTS ||--o{ MESSAGES_ARCHIVE : contains
  ...
  POSTS ||--o{ REPORTS : "reported as"
  SESSIONS ||--o{ REPORTS : files
  SESSIONS ||--o{ SANCTIONS : receives
  USERS ||--o{ SANCTIONS : receives
  POSTS ||--o| PREMIUM_POSTS : "upgraded to"
  PREMIUM_POSTS ||--o{ PAYMENTS : "receives"
...
> **설계 원칙**: `posts` 테이블에는 게시물의 상태(LIVE/READ)를 컬럼으로 저장하지 않습니다. 상태는 `created_at` 기준으로 항상 재계산하며(PRD §4.2, 아키텍처 §3.2 이중 검증 원칙), 이를 통해 DB와 애플리케이션 간 상태 불일치 문제를 원천 차단합니다.
...
CREATE INDEX idx_posts_created_at ON posts(created_at); -- Lifecycle/Purge Worker 스캔용
...
CREATE INDEX idx_msg_archive_post_id ON messages_archive(post_id, created_at);
...
CREATE UNIQUE INDEX idx_noti_dedup_user ON notifications_log (
    user_id, post_id, notification_type, sent_date
) WHERE user_id IS NOT NULL;
-- PRD §7.2: 동일 세션-게시물-유형 1일 1회 제한 (익명 유저용)
CREATE UNIQUE INDEX idx_noti_dedup_session ON notifications_log (
...
| 파티션 삭제 | 48시간 경과 데이터는 Purge Worker가 **개별 행 삭제가 아닌 파티션 단위 DROP**으로 처리하면 성능상 유리하나, 버즈48은 게시물 단위 생성 시점이 제각각이라 **행 단위 삭제가 불가피**. 대량 삭제 시 `DELETE ... WHERE created_at < now() - interval '48 hours'`를 배치 사이즈(500건)로 나누어 실행(아키텍처 문서 §3.3과 일치) |
...
| `post:{post_id}:messages` | Stream | LIVE 메시지 원본. `XADD`로 추가, `XRANGE`로 재연결 sync | READ 전환 시 PG 반영 확인 후 삭제 |
| `post:{post_id}:conn_count` | String(INCR/DECR) | 실시간 동접자 수 | 연결/해제 시 갱신, 60초 지연 해제(§4.3 어뷰징 방어) |
| `post:{post_id}:msg_times` | Sorted Set(score=timestamp) | 최근 5분 채팅 수 계산용 | `ZREMRANGEBYSCORE`로 5분 이전 항목 주기 제거 |
| `post:{post_id}:reaction_times` | Sorted Set | 최근 5분 반응 수 계산용 | 동일 |
| `hot:ranking` | Sorted Set(score=HOT Score) | 전체 게시물 HOT 순위 | 10초 주기 전체 갱신 |
| `post:{post_id}:reactions:{message_id}` | Set(session_id 목록) | 메시지별 반응 유저 집합(1인 1반응 검증) | READ 전환 시 PG 이관 |
| `rate_limit:post_creation:{session_id}` | String(counter) | 1시간 3개 게시물 작성 제한(POST-03) | 60분 슬라이딩, TTL 갱신 방식 |
| `disconnect_pending:{session_id}:{post_id}` | String | 재연결 유예 플래그 | TTL 60초 |
| `post:{post_id}:state_cache` | String | LIVE/READ 상태 캐시(조회 성능용, 판정의 진실은 아님) | Lifecycle Worker 10초 주기 갱신 |
| `pubsub:post:{post_id}` | Pub/Sub 채널 | WebSocket 멀티 인스턴스 팬아웃. 메시지 발행(`PUBLISH`) → 각 WS 서버 구독(`SUBSCRIBE`) 후 연결된 클라이언트에 브로드캐스트 | 영구 보관 없음, 구독 중인 인스턴스가 없으면 메시지 소실 |
